...
/**
 * Send CSV with correct headers. If `rows` is empty and `columns` are provided,
 * we still send a header line (useful for ArcGIS/Excel).
 */
function sendCsv(res, filename, rows, columns) {
  ...
  if (!rows || rows.length === 0) {
    // Only header line if we know the columns; otherwise empty file
    return res.send(cols.length ? cols.join(",") + "\n" : "");
  }
...
router.get("/daily.csv", async (_req, res) => {
  try {
    ...
    sendCsv(res, "daily.csv", shaped, ["day", "status", "priority", "count"]);
  } catch (e) {
    console.error("[analytics] /daily.csv error:", e);
    res.status(500).end();
  }
});
...
router.get("/by-asset.csv", async (_req, res) => {
  try {
    ...
    sendCsv(res, "by-asset.csv", shaped, ["assetId", "count"]);
  } catch (e) {
    console.error("[analytics] /by-asset.csv error:", e);
    res.status(500).end();
  }
});
...
router.get("/sla.csv", async (_req, res) => {
  try {
    ...
    sendCsv(res, "sla.csv", shaped, ["day", "breaches"]);
  } catch (e) {
    console.error("[analytics] /sla.csv error:", e);
    res.status(500).end();
  }
});
